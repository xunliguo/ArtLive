package com.example.rh.artlive.richtext;
		...
		for (int index = 0; index < num; index++) {
			...
			if (itemView instanceof EditText) {
				...
				DataImageView item = (DataImageView) itemView
						.findViewById(R.id.edit_imageView);
//				testUpload(item.getAbsolutePath());
				itemData.mSDPath = item.getAbsolutePath();
				itemData.imagePath = testUpload(item.getAbsolutePath());
				com.example.rh.artlive.util.Log.e("路径"+item.getAbsolutePath());
	...
	public String testUpload(String path) {
		try {
			...
			com.example.rh.artlive.util.Log.e("字节流"+uploadBuffer);
			fis.close();//这两行原来没有
			baos.flush();
			return uploadBuffer;
