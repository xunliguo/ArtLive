...
public class GiftItemView extends LinearLayout {
    ...
    public void setGift(LiveGiftBean gift) {
        this.gift = gift;
        refreshView();
    }
    ...
    public void refreshView(){
        ...
        if (!TextUtils.isEmpty(gift.getPrice())){
            Glide.with(getContext()).load(gift.getPoster()).placeholder(R.drawable.default_head).into(avatar);
        }else {
...
//        giftIv.setImageResource(gift.giftType);
        Glide.with(context).load(gift.getPoster()).into(giftIv);
    ...
    public interface OnAnimatorListener{
        public void onAnimationEnd(LiveGiftBean gift);
        public void onAnimationStart(Animator animation);
    }
}
